...
| Path | Role |
|---|---|
| services/api/src/server.js | The service proper: env config, `initDb`, `initCache` with `cacheGet`/`cacheSet`/`cacheDrop`, the `health` handler, the `apiRoutes` plugin (tasks / share / quiz), `findTask`, `QUIZZES`, `quizClients`, and the `start` sequence |
| services/api/src/queue.js | Optional RabbitMQ publisher: `initQueue`, `publish`, `queueName`, and `connectOptions` (explicit PLAIN credentials from `RABBITMQ_PASSWORD`) |
| services/api/src/audit.js | Optional MongoDB audit log: the `AuditEvent` mongoose model, `initAudit`, `recordEvent`, `auditCollection` |
...
## Notes
- **`DATABASE_URL` is projected twice in `build.yaml`**: by `requires.db` (postgres, `url`) and again by `requires.db-mongodb` (mongodb, `url`). Only one value can reach the pod, and a Mongo connection string handed to `pg.Pool` breaks every task route. `db-mongodb` should project `MONGODB_URL` only — it arrived with the keycloak commit, whose message never mentions it.
- **`RABBITMQ_PASSWORD` and `TASKS_QUEUE` are read by `queue.js` but declared nowhere in `build.yaml`**, so they are never injected: in preview the publisher authenticates with whatever the URL carries and always uses the `orbit.tasks` default. An env the code reads and the spec omits fails silently.
...
- **`publish()` is never called.** `server.js` imports only `initQueue`, so the broker connects and asserts the queue but nothing is ever published. Wiring a task event is the missing half of that feature.
