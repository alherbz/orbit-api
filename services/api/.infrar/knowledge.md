...
- The `cache` requirement asks for `tenancy: db-index`, because the cache key `tasks:all` is a constant in the code: two previews sharing one Redis without a unit would overwrite each other's task list. A db index is the unit that isolates them with no code change — node-redis honours the `/N` a `redis://host:port/N` URL carries.
