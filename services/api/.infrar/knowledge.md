...
## Notes
The image build requires package-lock.json to be present and in sync with package.json (`npm ci` fails otherwise); update the lockfile whenever dependencies change. The base image was briefly pinned to the nonexistent tag `node:20.99.99-alpine` (which broke image builds) and has been reverted to `node:20-alpine`. The in-memory task store is per-process and resets on restart — it exists only as a no-database fallback.
